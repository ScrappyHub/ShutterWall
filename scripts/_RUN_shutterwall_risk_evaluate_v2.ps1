...
foreach ($device in $devices) {
  ...
  if ($latestReviewByDevice.ContainsKey($deviceId)) {
    $review = $latestReviewByDevice[$deviceId]
    $reviewDisposition = [string]$review.disposition
  }
  ...
  if ($reviewDisposition) {
    $evidenceBase += ("operator_disposition:" + $reviewDisposition)
  }

  if ($reviewDisposition -eq "confirmed_camera") {
    Add-RiskFinding -Findings $allFindings `
      -DeviceId $deviceId `
      -Severity "medium" `
      -RuleId "SW.RISK.OPERATOR_CONFIRMED_CAMERA.V1" `
      -Title "Operator confirmed camera-class device" `
      -Reason "An operator explicitly confirmed this device is a camera-class device." `
      -EvidenceRefs (@($evidenceBase)) `
      -RecommendedAction "Prioritize this device for privacy control and enforcement planning." `
      -Enforceable $true
  }
  elseif ($reviewDisposition -eq "needs_review") {
    Add-RiskFinding -Findings $allFindings `
      -DeviceId $deviceId `
      -Severity "low" `
      -RuleId "SW.RISK.OPERATOR_REVIEW_PENDING.V1" `
      -Title "Operator review pending" `
      -Reason "An operator marked this device for review before stronger actions are taken." `
      -EvidenceRefs (@($evidenceBase)) `
      -RecommendedAction "Complete operator review before live enforcement." `
      -Enforceable $false
  }

  if ($reviewDisposition -ne "not_camera") {
...
$riskDoc = [ordered]@{
  schema = "shutterwall.risk.collection.v2"
  risk_run_id = $riskRunId
...
Write-Host "SHUTTERWALL_RISK_EVALUATE_V2_OK" -ForegroundColor Green
