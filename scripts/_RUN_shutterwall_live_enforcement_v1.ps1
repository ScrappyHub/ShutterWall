...
function Add-EnforcementReceipt {
  param(
    [Parameter(Mandatory=$true)][System.Collections.ArrayList]$Receipts,
  ...
  $receipt = [ordered]@{
    schema = "device.enforcement.receipt.v1"
...
$LiveReceiptPath  = Join-Path $RunRoot "device.enforcement.live.receipts.v1.json"
...
Append-NdjsonLine -Path $ReceiptPath -Object ([ordered]@{
  ...
  event = "live_enforcement_started"
  run_root = $RunRoot
  plan_path = $PlanPath
  mode = $mode
...
foreach ($action in $actions) {
  ...
  if ($supported -notcontains $actionType) {
    Add-EnforcementReceipt -Receipts $liveReceipts `
      ...
      -Mode $mode `
      -Result "skipped" `
      -Detail "Action type not mapped to live firewall enforcement in v1." `
      -RuleName ""
    continue
  }
  ...
  if (@($existing).Count -gt 0) {
    Add-EnforcementReceipt -Receipts $liveReceipts `
      ...
      -Detail "Firewall rule already exists." `
      -RuleName $ruleName
    continue
  }
  ...
  New-NetFirewallRule `
    ...
    -Enabled True `
    -Profile Any `
    -RemoteAddress $targetIp | Out-Null
  ...
  Add-EnforcementReceipt -Receipts $liveReceipts `
    ...
    -Mode $mode `
    -Result "applied" `
    -Detail "Created outbound Windows Firewall block rule for remote IP." `
    -RuleName $ruleName
}
...
$liveDoc = [ordered]@{
  schema = "shutterwall.enforcement.live.receipts.collection.v1"
  run_root = $RunRoot
...
Append-NdjsonLine -Path $ReceiptPath -Object ([ordered]@{
  ts_utc = [DateTime]::UtcNow.ToString("o")
  schema = "shutterwall.receipt.v1"
  event = "live_enforcement_completed"
  run_root = $RunRoot
...
Write-Host "SHUTTERWALL_LIVE_ENFORCEMENT_V1_OK" -ForegroundColor Green
